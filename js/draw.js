...
const originalInformation = [
    ...
    ...[`發現具名衛星：`, '*6', '-', '^'],
    ...[`各國參加：`, '*27', '-', '^'],
    `卡西尼－惠更斯號是第一個去：^*- orbit Saturn^*- land in the outer solar system^*- sample an extraterrestrial ocean`, '-', '^',
    `它揭示了泰坦是我們遇到過的最像地球的世界之一`, '-', '^',
    `（泰坦是環繞土星運行的一顆衛星）`, '-', '^',
    cpxaisfm(['查看=', {
        tag: 'a',
        attributes: {
            href: 'https://solarsystem.nasa.gov/missions/cassini/overview/',
            target: '_blank'
        }
    }, 'solarsystem.nasa.gov', {
        end: true
    }, `=更多細節`]), '-', '^',
    cpxaisfm(['用來自美國太空總署（ NASA ） 的 3D 資料=', {
        tag: 'a',
        attributes: {
            href: 'https://nasa3d.arc.nasa.gov/models',
            target: '_blank'
        }
    }, '到這裡', {
        end: true
...
let information = [...originalInformation];

async function infoAdder() {
    ...
    const item = information.shift();
    const isString = typeof item === 'string';
    if (isString && item === '-') {
        setTimeout(infoAdder, 1500);
        return;
    }
    let arr = [];
    if (isString) {
        arr = item.split('').flatMap(x => {
            if (x === '*') return ['&nbsp;', '&nbsp;', '&nbsp;', '&nbsp;'];
            if (x === '=') return '&nbsp;';
            if (x === '^') return '<br/>';
            return x;
